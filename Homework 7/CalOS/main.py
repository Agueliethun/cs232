...
class Monitor:
    ...
    def _handle_data_label(self, addr, line, pcb):
    	...
    	if len(line.split()) != 2:
    		raise ValueError("Illegal format: __data: must be followed by the high memory address")
    	high_addr = int(line.split()[1]) + addr
    	pcb.set_high_mem(high_addr)
    	if self._debug:
    		print("__data : high memory set to " + high_addr)
